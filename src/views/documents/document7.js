...
<html lang="en">
  ...
  $(function() {
    ...
    sort.on("sortupdate", function( event, ui ) {
      const newOrder = [];
      document.querySelectorAll('section[website-builder]').forEach(el => {
        newOrder.push(el.getAttribute('website-builder'));
      });
  ...
  document.querySelectorAll('section[website-builder]').forEach(el => {
    el.addEventListener('click', () => {
      window.top.postMessage({
        event: 'click',
        blockId: el.getAttribute('website-builder')
      }, '*');
    })
  });
  </script>
...
const document = {
  hbs,
  name: 'Bootswatch Spacelab (Bootstrap 4.5)'
}
